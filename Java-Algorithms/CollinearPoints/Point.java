...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        if (this.y == that.y)
            return this.x == that.x ? Double.NEGATIVE_INFINITY : 0.0;
        else
            return this.x == that.x ? Double.POSITIVE_INFINITY :
                    (double) (this.y - that.y) / (this.x - that.x);
    }
